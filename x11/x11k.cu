	...
	do {
		...
		*hashes_done = pdata[19] - first_nonce + throughput;
		...
		if (work->nonces[0] != UINT32_MAX)
		{
			...
			gpulog(LOG_WARNING, thr_id, "vhash: %d", vhash);
			gpulog(LOG_WARNING, thr_id, "%d", vhash[7]);
			x11khash(vhash, endiandata);
			gpulog(LOG_WARNING, thr_id, "vhash: %d", vhash);
			gpulog(LOG_WARNING, thr_id, "%d", vhash[7]);
			gpulog(LOG_WARNING, thr_id, "htarg: %d", Htarg);
			gpulog(LOG_WARNING, thr_id, "ptarg: %d", ptarget);
			gpulog(LOG_WARNING, thr_id, "%d<=%d", vhash[7], Htarg);
		...
		// gpulog(LOG_WARNING, thr_id, "max_nonce: %d", max_nonce);
		// gpulog(LOG_WARNING, thr_id, "throughput: %d", throughput);
		// gpulog(LOG_WARNING, thr_id, "pdata: %d", pdata[19]);
		if ((uint64_t) throughput + pdata[19] >= max_nonce) {
			gpulog(LOG_WARNING, thr_id, "greater than max nonce here");
			pdata[19] = max_nonce;
			break;
		}
		pdata[19] += throughput;
